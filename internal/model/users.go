...
var UserRels = struct {
	RequesterRelationships string
	TargetRelationships    string
}{
	RequesterRelationships: "RequesterRelationships",
	TargetRelationships:    "TargetRelationships",
}

// userR is where relationships are stored.
type userR struct {
	RequesterRelationships RelationshipSlice `boil:"RequesterRelationships" json:"RequesterRelationships" toml:"RequesterRelationships" yaml:"RequesterRelationships"`
...
func (r *userR) GetRequesterRelationships() RelationshipSlice {
	if r == nil {
		return nil
	}
	return r.RequesterRelationships
}
...
// RequesterRelationships retrieves all the relationship's Relationships with an executor via requester_id column.
func (o *User) RequesterRelationships(mods ...qm.QueryMod) relationshipQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"relationships\".\"requester_id\"=?", o.ID),
	)
...
// LoadRequesterRelationships allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadRequesterRelationships(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`relationships.requester_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.RequesterRelationships = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.RequesterID {
				local.R.RequesterRelationships = append(local.R.RequesterRelationships, foreign)
				if foreign.R == nil {
					foreign.R = &relationshipR{}
				}
...
// AddRequesterRelationships adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.RequesterRelationships.
// Sets related.R.Requester appropriately.
func (o *User) AddRequesterRelationships(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Relationship) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			RequesterRelationships: related,
		}
	} else {
		o.R.RequesterRelationships = append(o.R.RequesterRelationships, related...)
	}
